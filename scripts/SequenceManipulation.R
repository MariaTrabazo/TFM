...
get_sequences_from_database <- function(){
    ...
    write(fetch_response, file = here("results",
...
align_two_sequences <- function(){
    ...
    list_sequences <- read_fasta("results", "sequence_list_r.fasta")
    ...
    write_output_txt("results", "results_alignment.txt", paste("R alignment score", score(alignment)))
...
get_complementary_reverse_sequence <- function(){
    ...
    write_output_txt("results", "results_rev_comp.txt", paste("R reverse complementary ", toupper(reverse_complementary)))
...
get_sequence_from_coordinates <- function(){
    ...
    list_sequences <- read_fasta("results", "sequence_list_r.fasta")
    write_output_txt("results", "results_coordinates.txt", "R sequences from coordinates")
    for(seq in list_sequences){
        ...
        write_output_txt("results", "results_coordinates.txt", toString(feature_seq))
    }
    end_time <- Sys.time()
    time_taken <- end_time - start_time
...
match_subsequence <- function(){
	...
	write_output_txt("results", "results_match.txt", "R subsequences matches")
	for (s in list_strings){
        count <- 0
        for(seq in list_sequences){
        	count <- count + str_count(seq, s)
        }
        write_output_txt("results", "results_match.txt", toString(count))
...
get_benchmarks <- function(){
	...
	write_results("results", "results_database.csv", df_database)
	...
	write_results("results", "results_alignment.csv", df_alignment)
	...
	write_results("results", "results_rev_comp.csv", df_rev_comp)
	...
	write_results("results", "results_coordinates.csv", df_coordinates)
	...
	write_results("results", "results_match_subsequence.csv", df_match_subsequence)
